...
describe("codex.terminal.snacks", function()
  ...
  describe("with snacks mock", function()
    ...
    it("open() converts argv command tables to a shell command", function()
      snacks_mod.open({ "codex", "--remote", "ws://127.0.0.1:19999" }, {})
      assert.equals(1, #toggle_calls)
      assert.matches("codex", toggle_calls[1])
      assert.matches("--remote", toggle_calls[1])
      assert.matches("ws://127.0.0.1:19999", toggle_calls[1])
    end)

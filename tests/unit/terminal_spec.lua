...
describe("codex.terminal", function()
  ...
  describe("setup() — provider resolution", function()
    ...
    it("selects external when provider='external'", function()
      terminal.setup({ terminal = { provider = "external" }, codex_cmd = "codex" })
      assert.equals("external", terminal._get_active_provider_name())
    end)
  end)
  ...
  describe("cmd_override parameter", function()
    before_each(function()
      terminal.setup({ terminal = { provider = "native" }, codex_cmd = "codex" })
    end)
    ...
    it("open(cmd_override) uses override", function()
      ...
      assert.equals("codex --remote ws://127.0.0.1:9999", mock_native._calls[1][2])
    end)
